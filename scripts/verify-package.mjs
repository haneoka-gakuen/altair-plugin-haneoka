...
for (const path of ["dist/index.js", "dist/index.d.ts", "LICENSE", "README.md", "SECURITY.md"]) {
...
for (const forbidden of ["@haneoka/bestdori", "packages/story", "packages/story-editor", "Live2DCubismCore"]) {
  if (distSource.includes(forbidden)) {
    throw new Error(`Published output contains forbidden host/runtime coupling: ${forbidden}`);
  }
}
